...
- **Smart plug set power tracks the power monitor**: A plug's allocated current is always derived from its set power. When a power-measurement entity is configured, the set power is updated from the live measured draw (lightly smoothed) while the plug is on, so it converges on the device's real consumption.
