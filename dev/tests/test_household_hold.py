...
# ---------------------------------------------------------------------------
# Module loading (same pattern as run_tests.py / test_auto_detect.py: the
...
if f"{_PKG_CALC}.utils" not in sys.modules:
    _load_module_as(f"{_PKG_CALC}.utils", _calc_dir / "utils.py")
_load_module_as(f"{_PKG_COMP}.const.common", _comp_dir / "const" / "common.py")
