...
- [ ] **Battery inverter device type** — the consumer of the PV clipping forecast sensors (`forecast_battery_max_soc`, `forecast_charge_limit_w`): a device type that optionally *writes* the hub's advisory values to a hybrid inverter's max-SOC / charge-rate registers (via `number`/`select` entities from the inverter's integration), with deadbands like the power station's. The hub itself stays advisory-only by design.
