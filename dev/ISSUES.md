...
8. ~~**Hub calc runs once per device entry per cycle — cycle-counted state advances N× too fast**~~ **FIXED 2026-08-17** — the calculation is now hub-owned: one `DataUpdateCoordinator` per hub entry runs `run_hub_calculation(hass, hub_entry)` exactly once per `site_update_frequency`, publishes hub_data (`entities/hub.py:publish_hub_data`), then awaits each registered load processor (`load_processors` registry, `entry_id` order — dispatch is now serialized). Engine API no longer takes an entity object. Regression tests: `test_one_calculation_per_cycle_with_three_loads`, `test_cycle_counted_engine_state_advances_once_per_cycle` (both fail on the old architecture), `test_site_cycle_publishes_hub_data_with_no_loads`. Note: serial dispatch means a site cycle takes the sum of per-load dispatch times — watch for slow-update warnings on large sites with slow OCPP services.
...
**Status (2026-08-17, same session): #18–30, #35, #36, #39, #42 FIXED** by delegated fix agents, plus the `hub_calculation.py` items of #43 (unused `voltage` param, dead `raw_solar`, `_read_grid_phases` return signature) — see the working-tree diff. All 161 calc scenarios pass on the combined tree; the Docker pytest tier was NOT run (Docker daemon unavailable this session) and should be run before release, particularly `test_config_flow*.py`, `test_init.py`, `test_sensor_update.py`. **Still open: #31, #32, #34, #37, #38, #40, #41, the remaining parts of #43 (`calculations/context.py`, `_create_entry_and_seed_options`), #44, #45, #46.** (#33 was fixed 2026-08-17 by the hub-coordinator refactor, together with #8.)
...
33. ~~**Hub-sensor staleness constant races the configurable cadence**~~ **FIXED 2026-08-17** — with the hub-owned coordinator (#8), `MockSensor`, the staleness fallback, and `_HUB_DATA_STALE_SECONDS` are deleted; `LoadJugglerHubSensor.async_update` is a pure read and the coordinator keeps hub_data fresh even on a hub with zero loads.
...
46. **Load sensor attributes `last_update` / `detected_phases` are never updated** *(found 2026-08-17 during the coordinator refactor; pre-existing)* — `entities/load.py` sets `self._last_update = datetime.min` and `self._detected_phases = None` in `__init__` and nothing ever writes them again, so the entity attributes permanently show `datetime.min` / `None`. Either wire them (set `_last_update` at the end of `async_process`, populate `_detected_phases` from the engine's `charger_active_phases`/auto-detect data) or drop them from `extra_state_attributes`.
