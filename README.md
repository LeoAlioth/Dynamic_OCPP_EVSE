...
| Recommended Battery Max SOC (%) | SOC ceiling that leaves exactly that much headroom **below where the battery was heading anyway** — your *Normal SOC ceiling source*, or 100% when none is configured — and rises back to that destination as the peak passes, so the battery still ends the day where you wanted it |
...
The written limit tracks *production*, not the meter — which is what keeps a boiling kettle or a passing cloud out of the register — and it uses your **Base consumption** as a stand-in for the house draw. A base set below the real draw would leave export permanently that far under the Excess trigger, so while the limit is engaged the advice is also nudged by a slow correction (10-minute time constant, hard-limited to ±400 W) until *reconstructed* export — the same draws-credited-back figure the Excess trigger uses, so your own loads can never steer it — settles on the trigger point. It stops accumulating whenever the advice sits on its floor or at full rate, and resets when the limit releases, so a cloudy hour costs nothing but the ten minutes it takes to earn back. Base consumption is unchanged where it belongs: in the clippable- and storable-energy figures.
