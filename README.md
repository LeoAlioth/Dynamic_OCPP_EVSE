...
| Recommended Battery Charge Limit (W) | Charge-rate cap protecting the reserved headroom **and the destination** — anchored one *Excess trigger margin* below the export limit, so export settles just under the limit and a hard-limiting inverter cannot mask the signal; at or above the destination it holds whatever the forecast says, and it releases to full rate only below the destination, and there only when nothing is left to clip or SOC is comfortably below the reserved ceiling |
...
The written limit tracks *production*, not the meter — which is what keeps a boiling kettle or a passing cloud out of the register — and it uses your **Base consumption** as a stand-in for the house draw. A base set below the real draw would leave export permanently that far under the Excess trigger, so while the limit is engaged the advice is also nudged by a slow correction (10-minute time constant, hard-limited to ±400 W) until *reconstructed* export — the same draws-credited-back figure the Excess trigger uses, so your own loads can never steer it — settles on the trigger point. It stops accumulating whenever the advice sits on its floor or at full rate — or whenever the battery is net *discharging*, since a charge limit cannot correct an export error the battery is causing — and resets when the limit releases, so a cloudy hour costs nothing but the ten minutes it takes to earn back. Base consumption is unchanged where it belongs: in the clippable- and storable-energy figures.
